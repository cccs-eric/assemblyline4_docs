...
| status | Enum | Reflects the current state of the alert throughout its lifecycle. This status is subject to change as a result of user actions, automated processes, or the execution of workflows within Assemblyline. The status provides insight into the current phase of analysis or response, with possible values: `ASSESS`, `MALICIOUS`, `NON-MALICIOUS`, `TRIAGE`, or None. | <div style="width:100px">:material-minus-box-outline: Optional</div> | `None` |
